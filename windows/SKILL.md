...
1. Run `scripts/install-theme-studio.ps1` once to set the matching official base colors and create launch/restore shortcuts.
2. Run `scripts/start-theme-studio.ps1`. Add `-RestartExisting` only when the user authorized restarting an already-open Codex app.
3. Run `scripts/verify-theme-studio.ps1 -ScreenshotPath <absolute-path>` after launch. Treat a missing hero, native composer, sidebar skin, or injection marker as failure. The native suggestion count is responsive and may be two to four.
...
5. Run `scripts/restore-theme-studio.ps1` for live removal. Add `-Uninstall` to delete shortcuts; add `-RestoreBaseTheme` when the user also wants the pre-install config backup restored.
...
- `assets/theme-studio.css`: full visual layer.
