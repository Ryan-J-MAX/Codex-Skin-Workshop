...
1. Run `scripts/install-skin-workshop.ps1` once to set the matching official base colors and create launch/restore shortcuts.
2. Run `scripts/start-skin-workshop.ps1`. Add `-RestartExisting` only when the user authorized restarting an already-open Codex app.
3. Run `scripts/verify-skin-workshop.ps1 -ScreenshotPath <absolute-path>` after launch. Treat a missing hero, native composer, sidebar skin, or injection marker as failure. The native suggestion count is responsive and may be two to four.
...
5. Run `scripts/restore-skin-workshop.ps1` for live removal. Add `-Uninstall` to delete shortcuts; add `-RestoreBaseTheme` when the user also wants the pre-install config backup restored.
...
- `assets/skin-workshop.css`: full visual layer.
