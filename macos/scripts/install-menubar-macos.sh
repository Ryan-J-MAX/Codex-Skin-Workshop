...
PLUGIN_SRC="$PROJECT_ROOT/menubar/codex_skin_workshop.10s.sh"
...
PLUGIN_DST="$PLUGIN_DIR/codex_skin_workshop.10s.sh"
{
  printf '%s\n' '#!/bin/bash'
...
if [ -d "$INSTALL_ROOT" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  /bin/cp -f "$PLUGIN_SRC" "$INSTALL_ROOT/menubar/codex_skin_workshop.10s.sh"
  /bin/chmod 755 "$INSTALL_ROOT/menubar/codex_skin_workshop.10s.sh"
  for name in pause-skin-workshop-macos.sh status-skin-workshop-macos.sh apply-from-menubar-macos.sh \
...
/bin/chmod 755 \
  "$PROJECT_ROOT/scripts/pause-skin-workshop-macos.sh" \
  "$PROJECT_ROOT/scripts/status-skin-workshop-macos.sh" \
